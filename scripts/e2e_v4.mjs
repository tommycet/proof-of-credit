// Run E2E on a specific contract address — patient polling
...
async function pollTx(txHash, label, maxWaitSec = 600) {
  ...
  for (let i = 0; i < iters; i++) {
    ...
    console.log(`  [${i*5}s] ${label} status=${r.statusName} result=${r.txExecutionResultName} votes=${vs}`)
    if (r.statusName === 'ACCEPTED' || r.statusName === 'FINALIZED') return r
    if (r.statusName === 'REJECTED') {
      console.log('REJECTED:', JSON.stringify(r, (k,v) => typeof v === 'bigint' ? v.toString() : v, 2).slice(0, 1500))
...
let appReceipt
try {
  appReceipt = await pollTx(applyTx, 'apply', 900) // 15 min — apply is slow
} catch (e) {
  console.log('Apply failed/timed out:', e.message)
  process.exit(1)
}
